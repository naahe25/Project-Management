        ...
        <div className="flex flex-col md:flex-row gap-4">
          {Object.keys(columns).map(col => (
            <Droppable droppableId={col} key={col}>
              {(provided) => (
                <div className="flex-1 bg-gray-100 rounded-2xl p-3 min-h-[300px] transition-colors duration-200 hover:bg-gray-200"
                    ...
                    <Draggable key={task.id} draggableId={String(task.id)} index={index}>
                      {(provided) => (
                        <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                          className={`bg-white rounded shadow p-3 mb-3 cursor-move hover:shadow-md transition-shadow duration-200 ${provided.draggableProps.style?.transform ? 'rotate-3 shadow-lg bg-blue-50' : ''}`}>
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-gray-400 text-lg hover:text-gray-600 transition-colors">⋮⋮</span>
